...
*/
     ...
     * @var array
     */
    protected $scriptTagPreSrcData = array();
     ...
     * @var array
     */
    protected $scriptTagPostSrcData = array();
    ...
    protected $registeredCssFiles = array();
     ...
     */
    public function registerJS($file, $template = false, $preSrcData = '', $postSrcData = '')
    {
        ...
        $this->scriptTagPreSrcData[$scriptHash] = $preSrcData;
        $this->scriptTagPostSrcData[$scriptHash] = $postSrcData;
     ...
     *                              See $this->available for schema
     *                              definition.
     */
    public function registerJsLibrary($name, $definition = array()) {
        $this->available[$name] = $definition;
    }
...
//        $specialcode = array();
        ...
        if (count($this->active) > 0 && $parseForMainTemplate) {
                    ...
                    foreach ($data['lazyDependencies'] as $dependency) {
                        ...
                        if (!empty($this->available[$dependency]['cssfiles'])) {
                            $cssfiles = array_merge(
                                $cssfiles,
                                $this->getRealCssFiles(
                                    $this->available[$dependency]['cssfiles']
                                )
                            );
                        }
                    }
            ...
            foreach (array_reverse($this->active) as $name) {
                ...
                if (!empty($data['cssfiles'])) {
                    $cssfiles = array_merge(
                        $cssfiles,
                        $this->getRealCssFiles($data['cssfiles'])
                    );
                }
        ...
        if ($parseForMainTemplate) {
            ...
            $jsScripts[] = $this->makeJSFiles($this->templateJS, true);
     ...
     * @access protected
     */
    protected function makeJSFiles($files, $template = false)
    {
        global $_CONFIG;
        $code = '';
        ...
        foreach ($files as $file) {
            ...
            $scriptHash = md5($file . $template);
            $preSrcData = '';
            if (isset($this->scriptTagPreSrcData[$scriptHash])) {
                $preSrcData = $this->scriptTagPreSrcData[$scriptHash];
            }
            $postSrcData = '';
            if (isset($this->scriptTagPostSrcData[$scriptHash])) {
                $postSrcData = $this->scriptTagPostSrcData[$scriptHash];
            }
    ...
    protected function makeCSSFiles($files)
    {
        ...
        foreach ($files as $file) {
            // The file has already been added to the js list
            if (array_search($file, $this->registeredCssFiles) !== false)
                continue;
            $this->registeredCssFiles[] = $file;
            $path = '';
    ...
    public function registerFromRegex($matchinfo)
    {
        ...
        if (!$alternativeFound) {
            $this->registerJS($script, true, $preSrcData, $postSrcData);
        }
    }
    ...
    protected function grabComments(&$content)
    {
        ...
        // filter esi-includes
        $content = preg_replace_callback('#<esi:include src="([^"]+)" onerror="continue"/>#', array($this, '_storeComment'), $content);
    }
