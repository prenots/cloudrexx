...
class ComponentController extends \Cx\Core\Core\Model\Entity\SystemComponentController implements \Cx\Core\Event\Model\Entity\EventListener {
    ...
    public function onEvent($eventName, array $eventArgs) {
        ...
        $targetEntity = '';
        $targetId = '';
        if ($eventArgs['template']->getParseTarget()) {
            $targetComponent = $eventArgs['template']->getParseTarget(
            )->getSystemComponent()->getName();
            $targetEntity = get_class($eventArgs['template']->getParseTarget());
            $targetId = $eventArgs['template']->getParseTarget()->getId();
        }
