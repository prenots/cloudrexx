...
cx.internal.dependencyInclude(
    ...
    function() {
        ...
        var UI = function() {
            ...
            var $ = jQuery = cx.libs.jquery['jqueryui'];
            ...
            var forms = {
                ...
                get: function(id) {
                    if (id != undefined) {
                        return cx.libs.jquery['jqueryui']("#form-" + id);
                    }
                    return cx.libs.jquery['jqueryui']("form.cx-ui");
                },
                ...
                validate: function(form, skipMessage) {
                    ...
                    form.find("input,textarea,select").each(function(index, el) {
                        if (!cx.libs.jquery['jqueryui'](el).is(".chosen-choices *")) {
                            cx.libs.jquery['jqueryui'](el).trigger("keyup");
                        }
                    });
                ...
                fix: function() {
                    var ids = [];
                    var needFix = [];
                    cx.libs.jquery['jqueryui'].each(cx.ui.forms.get(), function(index, el) {
                        var id = cx.libs.jquery['jqueryui'](el).attr("id").split("-")[1];
                        if (ids.indexOf(id) != -1) {
                            needFix.push(el);
                            return;
                        }
                    ...
                    var i = ids.sort().pop();
                    i++;
                    cx.libs.jquery['jqueryui'].each(needFix, function(index, el) {
                        cx.libs.jquery['jqueryui'](el).attr("id", "form-" + i);
                ...
                updateRelation: function(element, internal) {
                    if (internal !== undefined) {
                        ...
                        if (0 > cx.libs.jquery['jqueryui'].inArray(element.prop("tagName").toLowerCase(), [
                            "input",
                            "select",
                            "textarea"
                        ])) {
                        ...
                        internal = true;
                        element = cx.libs.jquery['jqueryui'](element);
                    }
                    ...
                    cx.ui.forms.updateRelation(cx.libs.jquery['jqueryui'](relationId), show);
                }
            };
