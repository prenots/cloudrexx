...
class ContrexxUpdate
{
    ...
    private function showInfoAboutLicense()
    {
        ...
        if (
            (
                isset($_POST['updateNext']) &&
                !empty($_POST['update_license_info'])
            ) ||
            // if installed version is newer then version 4,
            // then we can skip info about new license
            !$this->_isNewerVersion($_CONFIG['coreCmsVersion'], '4.0.0')
        ) {
    ...
    private function showInfoAboutLicensePage()
    {
        ...
        $this->objTemplate->setVariable(array(
            ...
            'UPDATE_NEW_VERSION'                => $this->getLiteralRepresentationOfVersion($_SESSION['contrexx_update']['version']),
        ));
    ...
    {
        global $_CONFIG, $_CORELANG, $opCacheEnabled, $opCacheEngine;
        ...
        if (!$opCacheEnabled) {
            $arrPhpExtensions['opcache']['class'] = 'warning';
            $arrPhpExtensions['opcache']['value'] = $_CORELANG['TXT_UPDATE_NO'];
        } else {
            $arrPhpExtensions['opcache']['class'] = 'successful';
            $arrPhpExtensions['opcache']['value'] = $_CORELANG['TXT_UPDATE_OP_CACHE_' . strtoupper($opCacheEngine)];
        }
        $arrPhpExtensions['opcache']['name'] = $_CORELANG['TXT_UPDATE_OP_CACHE'] . ' <span class="icon-info tooltip-trigger"></span><span class="tooltip-message">' . $_CORELANG['TXT_UPDATE_OP_CACHE_TOOLTIP'] . '</span>';
