...
Class Voting {
            ...
            if ($objVoting !== false && $objVoting->RecordCount() == 1) {
                if ($objVoting->fields['submit_check'] == 'email') {
                    ...
                    if ($objValidator->isEmail($email)) {
                        if (!$this->_alreadyVotedWithEmail($votingId, $email)) {
                            if (($msg = $this->VotingSubmitEmail($votingId, $voteId, $email)) === true) {
                                $msg = '';
                                $voted = true;
                            } else {
        ...
        if ($objResult->RecordCount() == 0) {
           ...
           $objTpl->setVariable(array(
            ...
            if (!$objResult->EOF) {
                ...
                $additional_fields = $this->_create_additional_input_fields($objResult);
                $objResult->MoveNext();
            } else {
                errorHandling();
                return false;
            ...
            $votingResultText = $this->_vote_result_html($votingId);
     ...
     * @return string
     */
    function setVotingResult(\Cx\Core\Html\Sigma $template)
    {
        ...
        $template->setVariable(array(
            'VOTING_RESULTS_TOTAL_VOTES' => contrexx_raw2xhtml($votingVotes),
            'VOTING_TITLE'               => contrexx_raw2xhtml($objResult->fields['question']),
            'VOTING_RESULTS_TEXT'        => $votingResultText
        ));
        $template->parse('voting_result');
    ...
    {
        if ($arrMxRRs = $this->_getMXHosts($email)) {
